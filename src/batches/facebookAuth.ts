...
export default async function install(toolbox: IgniteToolbox, templateProps: TemplateProps) {
  ...
  if (templateProps.useFacebookAuth) {
    ...
    const spinner = print.spin('Installing Facebook sdk')
    try {
      ...
      spinner.succeed('Facebook sdk\'s installed')
    }catch (e) {
      spinner.fail('Facebook sdk could not install')

      ignite.log(e)
      // if something go wrong, ignore the rrror
    }
